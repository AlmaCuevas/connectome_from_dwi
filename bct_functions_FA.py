...
def bct_functions(W_cmatrix, ID_nm, root_path):
    with np.errstate(divide='ignore', invalid='ignore'): # To ignore 1/0 warnings
        ...
        if isinstance(W_cmatrix, np.ndarray):
            ...
            W = pd.read_csv(W_cmatrix, header=None).to_numpy()
        else:
        ...
        betweenness=bct.betweenness_wei(W) # Node betweenness centrality is the fraction of all shortest paths in the network that contain a given node. Nodes with high values of betweenness centrality participate in a large number of shortest paths. L : NxN np.ndarray directed/undirected weighted connection matrix
...
def usage():
    ...
    temp = '''
    Calculate the output values from the BCT.

        Example call:
            bct_functions_FA.py <%s input file> <%s prefix name>


    <%s numpy array/.csv/.mat file>   = File that contains Undirected connectome
    <%s prefix of output>  = Output file prefix name. Will output "prefix_bct_type.mat" .csv file.
